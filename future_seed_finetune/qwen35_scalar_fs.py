...
def _projection_target_enabled(prefix: str, cfg: ScalarFutureSeedConfig) -> bool:
    targets = str(cfg.projection_lora_targets).lower()
    if targets == "both":
...
def apply_scalar_future_seed(model: nn.Module, cfg: ScalarFutureSeedConfig) -> nn.Module:
    ...
    for idx, layer in enumerate(backbone.layers):
        if getattr(layer, "layer_type", None) == "linear_attention" and idx >= cfg.start_layer:
            ...
            if cfg.projection_lora_rank > 0 and (
                (_projection_target_enabled("z", cfg) and not hasattr(layer.linear_attn, "z_lora_A"))
                or (_projection_target_enabled("out", cfg) and not hasattr(layer.linear_attn, "out_lora_A"))
            ):
                ...
                if _projection_target_enabled("z", cfg) and not hasattr(layer.linear_attn, "z_lora_A"):
                    layer.linear_attn.register_parameter(
                        "z_lora_A",
                        nn.Parameter(
                            torch.randn(
                ...
                if _projection_target_enabled("out", cfg) and not hasattr(layer.linear_attn, "out_lora_A"):
                    layer.linear_attn.register_parameter(
                        "out_lora_A",
                        nn.Parameter(
                            torch.randn(
